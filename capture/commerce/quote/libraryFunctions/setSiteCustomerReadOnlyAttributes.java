...
Updates:    
20141007 John Palubinskas - Added contID_readOnly_quote for contID F/O indicator
20141218 John Palubinskas - Upgrade fix for split function
20150123 John Palubinskas - #233 set the fees from the data tables rather than from the
                            checkboxes since this is for the read only attributes.  Refactored for clarity.
20150515 Ryan Nabor       - #291 query divisionFeeRate to determine if admin fee is charged for new customer
20150526 Gaurav Dawar     - #573 Fixed Fee percentage in the header and CSA.
20150603 JPalubinskas     - #552 set new/new to new in salesActivity_RO_quote
...
if(salesActivity_quote <> ""){
    salesActivityRO = salesActivity_quote;
    if (salesActivity_quote == "New/New") { salesActivityRO = "New"; }  // set the header display for New/New to be New

    retStr = retStr + "1~salesActivity_RO_quote~" +  salesActivityRO + "|";
}
